...
-- FUNCTION: public.get_harvest_views_attempts_for_user(integer)
...
CREATE OR REPLACE FUNCTION public.get_harvest_views_attempts_for_user(
	integer)
    RETURNS TABLE(id integer, prices jsonb, total jsonb, invitations integer) 
    LANGUAGE 'sql'
    COST 100
    VOLATILE PARALLEL UNSAFE
    ROWS 1000
...
AS $BODY$
   ...
   SELECT id, prices, maps[4]->'result' as total, invitations from harvest where user_id = $1
$BODY$;

ALTER FUNCTION public.get_harvest_views_attempts_for_user(integer)
    OWNER TO postgres;
